...
In reading raw files, I added ID information (\textsf{\footnotesize \Sexpr{gsub("\\_", "\\\\_", grepout("Abu\\.", fn))}}) to all pages. I further added HH ID information from the admin file.
...
@
Tabulation of arms with \textsf{ar.0}. There are \Sexpr{sum(is.na(ar.0[, .(teenum =1:.N, RArm), by = .(hhid, survey)][survey == 1 & teenum == 1, RArm]))} NAs which will be filled in with \textsf{RCT\_village.dta} with \textsf{ar, arAll} in the next subsection.
...
setkey(ros.0, hhid)
# arAll is admin data with survey round info + roster
arAll <- ros.0[ar.00]
...
arAll[, group.id := NULL]
<<arAll starts from survey 1 or 2>>=
table0(arAll[MemNum == 1 & grepl("old|iRej|^g", Mstatus) & 
  !grepl("tw|dou", TradGroup), .(survey, RArm)])
@

There are \Sexpr{nrow(ar.1[is.na(DisDate1) & survey == 1 & MemNum == 1 & grepl("old", Mstatus), ])} members (\textsf{oldMember} in \textsf{Mstatus}), 
\Sexpr{nrow(ar.1[is.na(DisDate1) & survey == 1 & MemNum == 1 & grepl("new", Mstatus), ])} members (\textsf{newGroup} in \textsf{Mstatus}),
\Sexpr{nrow(ar.1[is.na(DisDate1) & survey == 1 & MemNum == 1 & grepl("iRe", Mstatus), ])} members (\textsf{iRejection} and \textsf{iReplacement} in \textsf{Mstatus})
...
@
\Sexpr{nrow(ar.1[is.na(DisDate1) & is.na(creditstatus) & survey == 1 & MemNum == 1, .(survey, randomization, Arm, Mstatus)] )} NAs in \textsf{creditstatus} are \textsf{gErosion} and \textsf{gRejection}. Their arms are not recorded in survey data and they will be supplemented with \textsf{vr} (from \textsf{RCT\_vilage.dta}) later.
<<>>=
summary(ar.1[is.na(DisDate1) & is.na(creditstatus) & survey == 1 & MemNum == 1, 
  .(survey,  Arm, ObPattern, AttritIn = factor(AttritIn), Mstatus, Mgroup)] )
@
...
<<There are members iReplacement in Mstatus who did not borrow but only saved, eval = F>>=
...
ar.1[is.na(DisDate1) & MemNum == 1 & !is.na(creditstatus),
#is.na(DisDate1) & MemNum == 1 & grepl("old|new|Rep", Mstatus), 
  BorrowerStatus := "pure saver"]
ar.1[is.na(DisDate1) & is.na(creditstatus) & MemNum == 1, 
  BorrowerStatus := "quit membership"]
ar.1[, BorrowerStatus := factor(BorrowerStatus)]
arAll[, BorrowerStatus := "borrower"]
arAll[is.na(DisDate1) & grepl("^dr", Mgroup) & grepl("N", creditstatus),
  BorrowerStatus := "quit membership"]
arAll[is.na(DisDate1) & grepl("old|new|Rep", Mstatus), 
  BorrowerStatus := "pure saver"]
arAll[, BorrowerStatus := factor(BorrowerStatus)]
table(arAll[MemNum == 1, BorrowerStatus])
@
...
<<contents of pure savers>>=
table0(ar[MemNum == 1 & survey == 1 & grepl("pu", BorrowerStatus), 
  RArm])
@
...
table0(arA[MemNum == 1,  .(survey, RArm)])
table0(arA[MemNum == 1,  .(survey, BorrowerStatus)])
table0(arA[MemNum == 1,  .(RArm, BorrowerStatus)])
arA[is.na(AssignOriginal), BorrowerStatus := "quit membership"]
table0(arA[MemNum == 1,  .(survey, BorrowerStatus)])
@
...
@
There are errors in repayment. \textsf{hhid} \Sexpr{as.character(unique(arA[o1600 == 1 & hhid %in% hhid[is.na(DisDate1) & value.repay > 0], hhid]))} recorded as repaid \Sexpr{arA[o1600 == 1 & hhid %in% hhid[is.na(DisDate1) & value.repay > 0] & MemNum == max(MemNum), value.repay]}, respectively, but with no record of disbursement and repayment before the final meeting. Change repayment to 0.
<<errors in repayment>>=
summary(arA[o1600 == 1 & hhid %in% hhid[is.na(DisDate1) & value.repay > 0], 
  .(hhid = factor(hhid), survey, DisDate1, ObPattern, 
  creditstatus, RArm, Mgroup, 
   Mstatus, GroupStatus, value.repay= factor(value.repay), 
   value.NetSaving = factor(value.NetSaving), BorrowerStatus)])
arA[o1600 == 1 & hhid %in% hhid[is.na(DisDate1) & value.repay > 0], 
  value.repay := 0]
@
After correcting the error, below gives the summary.
<<arA nonmember summary after correction>>=
summary(arA[o1600 == 1 & is.na(DisDate1) & MemNum == 1, 
  .(hhid = factor(hhid), survey, DisDate1, ObPattern, 
  creditstatus, RArm, Mgroup, 
   Mstatus, GroupStatus, value.repay= factor(value.repay), 
   value.NetSaving = factor(value.NetSaving), BorrowerStatus)])
...
<<Breakdown of first disbursement by RArm at rd 1 in roster plus admin 2, eval = F>>=
tb <- table0(ar[grepl("old|iRej|^g", Mstatus) 
...
tb <- round((tb/apply(tb, 2, sum))*1, 2)
rbind(tb, total = apply(tb, 2, sum))
...
<<Create Arm*HadCows Arm*HadCows*Time interactions in lvo, results = "hide">>=
...
<<fnames>>=
flnames <- c("Roster", "AllMeetingsRoster", "Asset", "Livestock", "LivestockProducts", "LabourIncome", "FarmRevenue", "Consumption", "Shocks")
@
