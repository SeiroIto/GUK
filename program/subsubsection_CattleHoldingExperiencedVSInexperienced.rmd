```{r ReadTrim NumCows for text, echo = F, cache = F, warning = F}
source(paste0(pathprogram, "ReadTrimLivestockByExperienceANCOVA.R"))
```
```{r tabulate Numcows, echo = F}
remove(list = ls(pattern = "^incl.?\\d"))
lvoEN = copy(lvoE) # all periods
lvoEN[, grepout("TotalImp.*Value$|Self|edV.*0", colnames(lvoEN)) := NULL]
#### consistency between cattle ownership/leaseship at baseline and 
#### livestock holding information
lvoEN[, Cattle := "None"]
lvoEN[OwnCattle == 1L, Cattle := "Owner"]
lvoEN[AdiCattle == 1L, Cattle := "Adi"]
if (CreateHTMLTable)
  knitr::kable(addmargins(table0(lvoEN[tee == 2, .(Cattle, NumCows0)])), 
    caption = "Number of cattle holding at baseline by experience")
```

<details><summary>Click here to see the code of cattle size impacts by experience.</summary>
```{r ReadTrim NumCows By experience, cache = F, warning = F}
source(paste0(pathprogram, "ReadTrimLivestockByExperienceANCOVA.R"))
remove(list = ls(pattern = "^incl.?\\d"))
lvoEN = copy(lvoE) # all periods
...
<details><summary>Click here to see the code of cattle size impacts by experience subgroups.</summary>
```{r numcows 2 separate subsample by experience}
...
<details><summary>Click here to see the code of results table on owned cattle of owners.</summary>
Number of owned cattle: Owners  
`r HTML_NumCowsByExperienceoTimeVarying`
</details>

<details><summary>Click here to see the code of results table on owned cattle of Adi.</summary>
Number of owned cattle: Adi  
`r HTML_NumCowsByExperienceaTimeVarying`
</details>

<details><summary>Click here to see the code of results table on owned cattle of others.</summary>
Number of owned cattle: Others  
`r HTML_NumCowsByExperiencenTimeVarying` 
</details>
