...
#### CLAUDE bug4: 2026-05-14 inverted condition: traditional+large got 190*45*2; should be gr/cat
adw[grepl("gr|cat", Arm), TotalDebt := as.integer(190 * MonthsOfRepayment * 2)]
...
adw[abs(ShortfallRate)==Inf | is.nan(ShortfallRate), ShortfallRate := NA]
...
\item[twice]	Two loans (roughly) equally split, 7840, 8960, or two disbursement dates are recorded and not amounts are not under the ``double.''
\end{description}
<<define TradGroup>>=
...
adw[(loanamount1st == 7840 & loanamount2nd == 8960) | 
  (!is.na(DisDate1) & !is.na(DisDate2) & is.na(DisDate3) & !(loanamount1st == 5600 & loanamount2nd == 11200)), 
  TradGroup := "twice"]
...
qsave(adw2, paste0(path1234, "admin_data_wide2.qs"))
