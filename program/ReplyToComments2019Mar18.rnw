...
<<all chunks0, cache = F, child='c:/data/GUK/analysis/program/ImpactEstimationOriginal1600_OptionSetting.rnw'>>=
@
...
\item	where? all interaction terms are positive and highly significant. [Referring to: Repayment is greater for \textsf{Large, LargeGrace, Cow} in rd 2-3 but they become statistically the same with \textsf{Traditional} in rd 3-4  (Table 4)]
	...
	\end{itemize}
\item	Later, use consistently either ``InKind" or ``InKind" throughout. 
	\begin{itemize}
	\vspace{1.0ex}\setlength{\itemsep}{1.0ex}\setlength{\baselineskip}{12pt}
	\item	I will correct it.
	\end{itemize}
\item	Better sort in the order of ``Large" ``With Grace" and ``InKind". 
	\begin{itemize}
	\vspace{1.0ex}\setlength{\itemsep}{1.0ex}\setlength{\baselineskip}{12pt}
	\item	I will.
	\end{itemize}
