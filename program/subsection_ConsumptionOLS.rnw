<<ReadTrimConsumption OLS, cache = F, warning = F>>=
...
<<consumption OLS regression set parameters original HHs>>=
...
<<consumption OLS select covariates original HHs>>=
source(paste0(pathprogram, "ConsumptionCovariateSelectionOLS.R"))
<<get jay from consumption OLS covariate selection>>=
...
\Sexpr{knit_child(paste0(pathprogram, "ANCOVAConsumptionOLSResults.rnw"))}
