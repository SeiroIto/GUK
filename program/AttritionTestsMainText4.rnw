...
TabLabelStrings[19:20] <- c("active.*race arms$", "active.*other arms$")
@
...
<<print trimmed sample attrition, eval = T, results = "asis">>=
for (i in 14:length(TabLabelStrings)) {
  ...
  if (grepl("active", TabLabelStrings[i])){
   ...
   cat(
   #gsub("survival\\\\", "attrited or rejected (NonActive) and other (Active) borrowers\\\\", 
     tblatt)
     #) 
   rm(tblatt)
  } else
  if (grepl("active|acti.*o", TabLabelStrings[i])) {
   tblatt <- eval(parse(text=paste0("Tb1", ii)))
