...
for (i in 1:length(datafiles))
  assign(datafiles[i], readRDS(paste0(pathsaveHere, "Roster", DataFileNames[i],
     "AdminOriginalHHsDataUsedForEstimation.rds")))
@
Find the 2nd batch of the treated.
<<Find the 2nd batch of the treated>>=
table(table(ar[!is.na(DisDate1), diff(DisDate1) > 100, by = groupid])[, "TRUE"])
