...
# items: rice, wheat, maize, potato, pulses, otherpulses, chicken, meat, fish, milk
#  egg, chili, banana, puffed, onion, garlic, ginger, oil, sugar, salt, tea, bettle, 
#  biscuit, cig, chewtobacco
for (i in items) {
  con[, paste0(i, "_UPrice") := 
    eval(parse(text = paste0(i, "_exp"))) / eval(parse(text = paste0(i, "_bought")))]
}
# Set time unit to annual. All expenditures are annualised values.
