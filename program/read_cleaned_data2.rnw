...
\item[NLHAssets]	\Sexpr{completeAsset} (\vfb+NLHAssetAmount := sum(amount[type %in% completeAsset], na.rm = T)+)
...
hasL[, Added := T]
hasL[, Added := any(grepl("y", purchase_in_last_1_year)), by = .(hhid, type)]
hasL[, EarliestResAmount := amount[!is.na(amount)][1], by = .(hhid, type)]
# Copy the EarliestResAmount (initial value) to all year if type-land has not been added in any year.
hasL[!Added & is.na(amount), amount := EarliestResAmount, by = .(hhid, type)]
@
...
<<echo = T>>=
hasL[hhid %in% hhid[Added] & type %in% type[Added], ][1:20+20*2]
# If ever purchased (in last 1 year of some round), assume that all value is acquired 
# in last 1 year and set baseline amount to zero
hasL[Added & year <= 2013, amount := 0, by = .(hhid, type)]
# Get total of: agricultural, residential, fallow/submerged land
hasL[, AmountFilled := sum(amount, na.rm = T), by = .(hhid, year, type)]
hasL[, EarliestAmount := AmountFilled[!is.na(AmountFilled)][1], by = .(hhid, type)]
