\section{Impact estimation results}
\label{AppendixEstimationTables}
\setcounter{table}{0}
\setcounter{figure}{0}

	In this section, the ANCOVA estimates on various outcomes using %\eqref{1steq} and 
	\eqref{2ndeq} are presented. In each table, the first column shows the covariate names and their means and standard deviations in the second column in the sample of the richest specification of the table. Specification (1) follows the most basic specification under \eqref{2ndeq}. From (2), we progressively add more covariates to control for the differences in initial conditions in an attempt to get more precise ANCOVA estimates. In \textsc{\footnotesize Figure \ref{fig IncomeConsumptionEffects}}, we use OLS estimates. It regresses on the intercept, $\bfdee_{i}$, and its period interactions. 
	
	\Sexpr{TabFNAncovaTop.c}
	



\subsection{Net assets}
\Sexpr{knit_child(paste0(pathprogram, "ANCOVANetAssetsResults2.rnw"))}
\Sexpr{
knit_child(paste0(pathprogram, "ANCOVANetBroadAssetsResults2.rnw"))
}

\subsection{Non land Assets}
\Sexpr{knit_child(paste0(pathprogram, "ANCOVANetNLAssetsResults2.rnw"))}

%\subsection{Livestock}
\Sexpr{
#knit_child(paste0(pathprogram, "ANCOVALivestockResults.rnw"))
}

\subsection{Cattle holding}
\Sexpr{knit_child(paste0(pathprogram, "ANCOVANumCowsResults2.rnw"))}

\subsection{Repayment}
\label{Sec App Repay}
\subsubsection{Saving and repayment}
\Sexpr{knit_child(paste0(pathprogram, "ANCOVARepaymentResults2.rnw"))}
\subsubsection{Shortfall}
\label{AppendixShortfall}
\Sexpr{knit_child(paste0(pathprogram, "ShortfallRegressionResults2.rnw"))}

\subsection{Schooling}
\Sexpr{knit_child(paste0(pathprogram, "ANCOVASchoolingResults2.rnw"))}

\subsection{Consumption}
\Sexpr{
#knit_child(paste0(pathprogram, "ANCOVAConsumptionResults.rnw"))
}
\Sexpr{knit_child(paste0(pathprogram, "OLSConsumptionResults.rnw"))}

\subsection{Income}
\Sexpr{knit_child(paste0(pathprogram, "ANCOVAIncomeResults.rnw"))}

\subsection{By experience}
\Sexpr{
#knit_child(paste0(pathprogram, "ANCOVANetAssetsExperienceResults.rnw"))
}
\Sexpr{knit_child(paste0(pathprogram, "ANCOVANetAssetsByExperienceResults2.rnw"))}
\Sexpr{
#knit_child(paste0(pathprogram, "ANCOVANumCowsExperienceResults.rnw"))
}
\Sexpr{knit_child(paste0(pathprogram, "ANCOVANumCowsByExperienceResults2.rnw"))}

\subsection{By poverty class}
\label{Subsec By poverty class}

\renewcommand{\arraystretch}{.6}
\mpage{\linewidth}{
\hfil\textsc{\footnotesize Table \refstepcounter{table}\thetable: Baseline variables of ultra poor relative to the moderately poor\label{tab UltraPoorVsModeratelyPoorBaselineVarsPermTestResults)}}\\
\setlength{\tabcolsep}{.5pt}
\setlength{\baselineskip}{8pt}
\renewcommand{\arraystretch}{.50}
\hfil\begin{tikzpicture}
\node (tbl) {\input{\Sexpr{
  paste0(pathsavePerm, "UltraPoorVsModeratelyPoorBaselineVarsPermTestResults.tex")
}}};
\end{tikzpicture}\\
\renewcommand{\arraystretch}{.8}
\setlength{\tabcolsep}{.1cm}
\hfil\begin{tabular}{>{\hfill\scriptsize}p{1cm}<{}>{\hfill\scriptsize}p{.25cm}<{}>{\scriptsize}p{12cm}<{\hfill}}
Notes: & 1. & \textsf{R}'s package \textsf{coin} is used for baseline group mean covariates to conduct approximate permutation tests. Number of repetition is set to \Sexpr{PermRepTimes}. \\
& 2. & See the footnote of \textsc{Table \ref{tab MainTextIRjecters}} for description of variables. 
\end{tabular}
}

\mpage{\linewidth}{
\hfil\textsc{\footnotesize Figure \refstepcounter{figure}\thefigure: Impacts of the ultra poor relative to the moderately poor\label{fig NetAssetsNLAssetsCattleUltraPoorEffects}}\\
\hfil\includegraphics[width = 12cm]{\Sexpr{    paste0(pathprogram, 
  "figure/EstimationMemo/NetAssetsNLAssetsCattleUltraPoorEffects.pdf")
}}\\
\renewcommand{\arraystretch}{1}
\hfil\begin{tabular}{>{\hfill\scriptsize}p{1cm}<{}>{\scriptsize}p{12cm}<{\hfill}}
Source: & Estimated with administrative and survey data in \textsc{Table \ref{tab NetAssetsTimeVaryingPovertyStatusANCOVA1}}. \\
Note:&  Impacts of ultra poor members - impacts of moderately poor members. \Sexpr{FNAssetDefinitions} \\[1ex]
\end{tabular}
}


\textsc{Figure \ref{fig NetAssetsNLAssetsCattleUltraPoorEffects}} shows net asset impacts are no different for the ultra poor borrowers at period 4. However, their time paths may be different. In \textsf{Large grace} and \textsf{Cattle} arms, the ultra poor borrowers accumulate smaller net assets in period 2 than their moderately poor counterparts of the same arms. Period 2 is the grace period and the repayment commence in the next period. The ultra poor borrowers may have saved up during the grace period to get ready for the installments of period 3 and 4. If this interpretation is correct, a grace period may have encouraged the ultra poor individuals to take up the loans. The ultra poor of \textsf{Large} arm does not show difference relative to the moderately poor borrowers. This hints that there may be self-selection of the ultra poor of the \textsf{Large} arm that they are better prepared for repayment than the ultra poor borrowers of \textsf{Large grace} and \textsf{Cattle} arms. There is no difference between arms for \textsf{Net non-livestock assets}, while the ultra poor of \textsf{Large} arm may have more cattle holding in period 4 than the moderately poor of the same arm. 

\textsc{Figure \ref{fig IncomeConsumptionPovertyEffects}} shows negative impacts on consumption relative to moderately poor members among \textsf{Large} and \textsf{Cattle} arms in period 4. This hints that repayment efforts may have resulted in consumption suppression for the ultra poor borrowers. Together with slower net asset accumulation, we may need to incorporate smaller, more distributed repayment installments and/or longer saving opportunities before the disbursement of loans for the ultra poor relative to the moderately poor. 

\Sexpr{knit_child(paste0(pathprogram, "ANCOVAByPovertyClassResults.rnw"))}


\renewcommand{\arraystretch}{.6}
\mpage{\linewidth}{
\hfil\textsc{\footnotesize Figure \refstepcounter{figure}\thefigure: Assets by period\label{fig AssetCumRelativeToConcurrentTradEffects}}\\
\hfil\includegraphics[width = 12cm]{\Sexpr{    paste0(pathprogram, "figure/EstimationMemo/AssetsByPeriod.pdf")}}\\
\renewcommand{\arraystretch}{1}
\hfil\begin{tabular}{>{\hfill\scriptsize}p{1cm}<{}>{\scriptsize}p{12cm}<{\hfill}}
Source: & Tabulated with survey data.\\
Note:&  Red squares are means of respective data. \Sexpr{FNAssetDefinitions} All net assets are in logarithms, number of cattle is in natural numbers. \\[1ex]
\end{tabular}
}


