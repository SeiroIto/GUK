...
for (k in 1:length(listheader)) {
  ...
  write.tablev(al.ltxtb, 
    paste0(pathsaveHere, "AssetLivestock", FileNameHeader[k], "OriginalHHsFDEstimationResults.tex"), 
    colnamestrue = F)
   # for slides
  ...
  write.tablev(sl.tab, 
    paste0(pathsaveHere, "ForSlide_AssetLivestock", FileNameHeader[k], "OriginalHHsFDEstimationResults.tex"), 
    colnamestrue = F)
}
