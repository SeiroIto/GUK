...
<<schooling FD read data test run original HHs>>=
...
teeArm
s.1 <- s1x[, .(RArm, groupid, Mstatus, Mgroup, hhid, tee, Fromxid)]
s1x <- s1x[(Fromxid), grepout("groupid|hhid|^mid$|RArm|sex|Eldest|tee|^dummy[A-Z]|Tim|RM|Head|Enrolled|Floo|Sch|xid$", 
  colnames(s1x)), with = F]
s2x <- s2x[(Fromxid), grepout("groupid|hhid|^mid$|RArm|sex|Eldest|tee|^dummy[A-Z]|Tim|RM|Head|Enrolled|Floo|Sch|xid$", 
  colnames(s2x)), with = F]
...
ds1x <- prepFDData(s1x, Group = "^HHMid$", TimeVar = "tee", Cluster = "groupid", 
  LevelCovariates = "^dummy[A-Z]|Head|Time|Fem|Floo|Eldest|xid", drop.if.NA.in.differencing = T,
