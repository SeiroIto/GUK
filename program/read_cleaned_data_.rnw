...
\section{Read files}
	...
	List and read files in following folders: \textsf{\footnotesize \Sexpr{gsub("\\_", "\\\\_", gsub("^.*RA", ".", path1234))}}, \gobblepars
<<list file names>>=
setwd(path1234)
...
\subsubsection{Create membership related variables}
...
@
Tabulate \textsf{AssignOriginal} in the first round.
...
\subsubsection{Supplement missingness in Arm info with village level info}

Supplement missingness in Arm info with village level info.
...
xidtemp[grepl("Rep", Mstatus), GroupStatus := "accepted"]
@
<<correct typos in interview dates>>=
# correct typos in interview dates
xidtemp[survey == 1 & grepl("092", st_day_inter), 
	st_day_inter := gsub("092", "09/2", st_day_inter)]
xidtemp[survey == 1 & grepl(";13", st_day_inter), 
	st_day_inter := gsub(";", "", st_day_inter)]
xidtemp[survey == 1 & grepl("--", st_day_inter), 
	st_day_inter := gsub("--", "-", st_day_inter)]
# there are 2 kinds of dates using "-" with YYYY and YY.
xidtemp[survey == 1 & grepl("-20..$", st_day_inter), 
	IntDate := as.POSIXct(asc(st_day_inter), format = "%d-%m-%Y")]
xidtemp[survey == 1 & grepl("-..$", st_day_inter), 
	IntDate := as.POSIXct(asc(st_day_inter), format = "%d-%m-%y")]
xidtemp[survey == 1 & grepl("\\/", st_day_inter), 
	IntDate := as.POSIXct(asc(st_day_inter), format = "%d/%m/%Y")]
xidtemp[survey >= 2, IntDate := as.POSIXct(asc(d_1st), format = "%d%m%y")]
xidtemp[survey >= 2 & is.na(IntDate) & !is.na(st_day) & !is.na(st_month) & !is.na(st_year), 
	IntDate := as.POSIXct(paste0(st_day, "-", st_month, "-", st_year), format = "%d-%m-%y")]
#xidtemp[, c("Disbursed1", "Disbursed2", "Disbursed3") := NA]
#xidtemp[is.na(IntDate)]
@
Create roster member order indicator, \textsf{MemNum}.
<<Create MemNum, echo = T>>=
setorder(xidtemp, hhid, survey, IntDate, mid)
xidtemp[, MemNum := 1:.N, by = .(hhid, survey, IntDate)]
xidtemp[, RosterMemTotal := .N, by = .(hhid, survey, IntDate)]
# HH member orders
table0(xidtemp[, .(MemNum, survey)])
table0(xidtemp[MemNum == RosterMemTotal, .(MemNum, survey)])
xidtemp[hhid %in% hhid[RosterMemTotal == 1], 
  .(hhid, survey, IntDate)]
@
Tabulation of \textsf{ArmAbu}.
<<Tabulation of ArmAbu>>=
table0(xidtemp[MemNum == 1, .(survey, ArmAbu)])
@
Tabulation of \textsf{AssignOriginal}.
<<Tabulation of AssignOriginal in xidtemp>>=
table0(xidtemp[MemNum == 1, .(survey, AssignOriginal)])
table0(xidtemp[MemNum == 1, .(AssignOriginal, ArmAbu)])
@
Create \textsf{Arm}.
<<Create Arm>>=
xidtemp[, Arm := AssignOriginal]
@
Tabulation of \textsf{Arm} after supplementing with \textsf{AssignOriginal} and \textsf{ArmAbu}.
<<supplementing Arm with AssignOriginal and ArmAbu, echo = T>>=
xidtemp[is.na(Arm) & !is.na(AssignOriginal), Arm := AssignOriginal]
xidtemp[is.na(Arm) & !is.na(ArmAbu), Arm := ArmAbu]
<<tabulate Arm after supplementing with AssignOriginal and ArmAbu>>=
table0(xidtemp[MemNum == 1, .(survey, Arm)])
@
Tabulation of \textsf{Arm} when dropping \textsf{twice, double} in traditional xidtempm.
<<Tabulation of Arm when dropping twice double in traditional xidtempm>>=
table0(xidtemp[MemNum == 1 & !grepl("tw|dou", TradGroup), .(survey, Arm)])
@
Tabulation of \textsf{Arm} when dropping \textsf{twice} in traditional xidtempm. This may make most sense but a lxidtempge attrition between rd 1 and 2.
<<Tabulation of Arm when dropping twice in traditional>>=
table0(xidtemp[MemNum == 1 & !grepl("tw", TradGroup), .(survey, Arm)])
@
Tabulation of \textsf{Arm} when dropping dirbursement after 2015-01-01. This has less attrition but includes heterogenous treatment among traditional.
<<Tabulation of Arm when dropping dirbursement after 2015 01 01>>=
table0(xidtemp[MemNum == 1 & as.Date(DisDate1) < as.Date("2015-01-01") , .(survey, Arm)])
#table0(xidtemp[MemNum == 1, .(Arm, Arm)])
#summxidtempy(xidtemp[MemNum == 1 & grepl("rop", Arm), ])
#summxidtempy(xidtemp[is.na(Arm), 1:10, with = F])
@
...
<<create LoanReject who reject loans yet stay as a member, echo = T>>=
xid[, LoanReject := F]
xid[is.na(DistDate1) & is.na(DistDate2) & is.na(DistDate3) & !grepl("dr", Mgroup), 
	LoanReject := T]
@
...
<<read Z for correcting sections>>=
Z <- readRDS(paste0(path1234, "data_read_in_a_list.rds"))
...
<<>>=
xlo[hhid == 7096201 & survey == 3, mrket_value := number_owned]
...
<<>>=
xlo[, LivestockCode := type]
table0(xlo[number_owned > 0 & mrkt_value == 0, .(year, LivestockCode)])
<<echo = F>>=
cowprice <- median(xlo[grepl("[Cc]ow", LivestockCode) & number_owned > 0 & sold_value > 0, 
	sold_value/number_owned])
oxprice <- median(xlo[grepl("[Oo]x", LivestockCode) & number_owned > 0 & sold_value > 0, 
	sold_value/number_owned])
goatprice <- median(xlo[grepl("[Gg]oat", LivestockCode) & number_owned > 0 & sold_value > 0, 
	sold_value/number_owned])
...
#xlo[grepl("ox", LivestockCode), ImputedValue := oxprice * number_owned]
xlo[grepl("Goa", LivestockCode), ImputedValue := goatprice * number_owned]
xlo[!grepl("co|ox|Goa", LivestockCode), ImputedValue := mrkt_value]
<<>>=
# Livestock assets.
...
@
Number of cows owned (\textsf{dummyHadCows}).
<<>>=
xlo[, dummyHadCows := 0L]
xlo[nowned_cow > 0 | nowned_ox > 0, dummyHadCows := 1L]
xlo[, dummyHadCows := as.integer(dummyHadCows[survey == 1] == 1L), by = hhid]
table(xlo[, .(survey, dummyHadCows)])
@
Owning at baseline (\textsf{dummyOwnCowsAtRd1}).
<<>>=
xlo[, dummyOwnCowsAtRd1 := 0L]
xlo[grepl("ow|ox", LivestockCode) & survey == 1 & number_owned > 0, dummyOwnCowsAtRd1 := 1L]
xlo[, dummyOwnCowsAtRd1 := as.integer(dummyOwnCowsAtRd1[survey == 1] == 1L), by = hhid]
table(xlo[, .(survey, dummyOwnCowsAtRd1)])
@
The number of cows owned prior to intervention:
<<>>=
table(xlo[dummyHadCows==1L & grepl("ow", LivestockCode) & survey==1, number_owned])
xlo[, NumCowsOwnedAtRd1 := 0L]
xlo[dummyHadCows==1L & grepl("ow", LivestockCode) & survey==1, 
  NumCowsOwnedAtRd1 := as.integer(number_owned)]
xlo[, NumCowsOwnedAtRd1 := as.integer(NumCowsOwnedAtRd1[1]), by = hhid]
xlo[, NumCows := 0L]
xlo[grepl("ow", LivestockCode), NumCows := as.integer(number_owned)]
# no need to copy within a round because there is only 1 line per rd
table0(xlo[, .(survey, NumCows)])
table0(xlo[survey == 1, .(cowox = nowned_cow+nowned_ox, NumCows)])
<<>>=
...
@
There are \Sexpr{sum(rxlx[, EverAttrit])} cases of attrition out of \Sexpr{nrow(rxlx[!grepl("dr", Mgroup), ])} non-dropout HHs in sample. Plot characteristics of attrited HHs in \textsf{\small Figure \ref{Figure HH attrition pattern}}. As can be seen, nothing seems to differ across arms for attrited HHs. 
...
reg.attrit2 <- glm(EverAttrit ~ HeadAge + HHsize + HAssetAmount + 
	...
	family = binomial(link = 'probit'))
roworder.item <- unlist(strsplit("Inter|^Hea|^HHsize$|^HA|^PA|Area|Ass", "\\|"))
attrit.coeff <- summary(reg.attrit1)$coeff
<<tabulate attrition regression results>>=
...
rn <- gsub("HAs.*", "Household asset (taka)", rn)
rn <- gsub("PAs.*", "Productive asset (taka)", rn)
rn <- gsub("^Ow.*", "Owned area (decimal)", rn)
rn <- gsub("^Op.*", "Operating area (decimal)", rn)
...
\input{c:/data/agri_SA/agri_census/program/tablecolortemplate_top_1row.tex}
\end{tikzpicture}\\
