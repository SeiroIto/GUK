...
#      )
  ...
  write.tablev(asp.ltxtb, 
    paste0(pathsaveHere, "Asset", FileNameHeader[k], "OriginalHHsFDEstimationResults.tex"), 
    colnamestrue = F)
   # for slides
  ...
  sl.ltxtb <- latextab(sl.tb, 
    ...
    separatingcoltitle = c("complete panel", "all panel"))
  sl.tab <- rbind(sl.ltxtb[-(nrow(sl.ltxtb)-(1:0)), , drop = F],
...
#     "\\cline{2-4} \\cline{6-7}\\\\",
  ...
  write.tablev(sl.tab, 
    paste0(pathsaveHere, "ForSlide_Asset", FileNameHeader[k], "OriginalHHsFDEstimationResults.tex"), 
    colnamestrue = F)
}
