...
<<echo = F>>=
NeAE1 <- readRDS(paste0(pathsaveHere, "NetAssetsExperienceANCOVA.rds"))
OwnCattleAtBaseline <- sum(NeAE1[tee == 1 & o800 == 1L, OwnCattle], na.rm = T)
AdiCattleAtBaseline <- sum(NeAE1[tee == 1 & o800 == 1L, AdiCattle], na.rm = T)
NoneCattleAtBaseline <- nrow(NeAE1[tee == 1 & o800 == 1L & OwnCattle == 0L & AdiCattle == 0L, ])
@
...
<<confi extract, echo = F>>=
# dNinT: nontrad - trad, in each period
ciN <- confi[grepl("^NetAssets$", FileName) & grepl("non.*- t.*d$", ImpactType) & grepl("Ta", regtype) & 
  num == 2 & period == 4,  ]
# dNinT: sum of (nontrad - trad), in each period
ciN <- confi[grepl("^NetAssets$", FileName) & grepl("^sum", ImpactType) & grepl("Ta", regtype) & 
  num == 2 & period == 4,  ]
# Net NL assets
ciNL <- confi[grepl("^NetNLAssets$", FileName) & grepl("^sum", ImpactType) & grepl("T$", regtype) & 
  num == 2 & period == 4,  ]
...
Second, it is the \textsf{Upfront} functional attribute that shows positive impacts on the net assets. This is consistent with the nonconvex production technology with fixed costs under a liquidity constraint, coupled with an inferior, smaller fixed cost production technology. \textsc{\normalsize Table \ref{tab ANCOVA narrow net assets timevarying}} [specification (2)] in the Appendix shows that, relative to the traditional microfinance lending, the upfront liquidity provision increases the net assets by BDT 
	\Sexpr{round(ciN[grepl("Up", attributes), estimate], 0)} 
	[CI \Sexpr{round(ciN[grepl("Up", attributes),  .(lb, ub)], 0)}] or \Sexpr{round((ciN[grepl("Up", attributes),  estimate]/NNASummary["stdNNA"]), 2)}$\sigma$ (of the baseline standard deviation) by the fourth year ($p$ value=\Sexpr{formatC(ciN[grepl("Up", attributes), pvalue]*100, format = "f", digits = 2)}\%). The grace period and in-kind loan with managerial supports do not add any marginal impacts beyond the upfront functional attribute 
(\Sexpr{round(ciN[grepl("Wi", attributes), estimate], 0)} [CI \Sexpr{round(ciN[grepl("Wi", attributes),  .(lb, ub)], 0)}] or \Sexpr{round((ciN[grepl("Wi", attributes),  estimate]/NNASummary["stdNNA"]), 2)}$\sigma$, $p$ value=\Sexpr{formatC(ciN[grepl("Wi", attributes), pvalue]*100, format = "f", digits = 2)}\% for grace period, 
\Sexpr{round(ciN[grepl("In", attributes), estimate], 0)} [CI \Sexpr{round(ciN[grepl("In", attributes),  .(lb, ub)], 0)}] or \Sexpr{round((ciN[grepl("In", attributes),  estimate]/NNASummary["stdNNA"]), 2)}$\sigma$, $p$ value=\Sexpr{formatC(ciN[grepl("In", attributes), pvalue]*100, format = "f", digits = 2)} for in-kind and managerial supports) 
by the fourth year. 
...
\hfil\includegraphics[width = 12cm]{\Sexpr{    
paste0(pathprogram,   
...
\end{figure}
	...
	The figure shows that, on average, the non-\textsf{Traditional} arms continue to own about .4 more cattle than the \textsf{Traditional} arm members conditional on the initial cattle holding, although estimates are indistinguishable from zero for some arms and regression specifications. In the row of \textsf{Net non-livestock assets}, a similar pattern is found and non-\textsf{Traditional} arms accumulate about 5000 BDT larger net non-livestock assets (\Sexpr{formatC(ciNL[grepl("ge$", attributes), estimate], format = "f", digits = 0)} BDT [\Sexpr{formatC(as.numeric(ciNL[grepl("ge$", attributes), .(lb, ub)]), format = "f", digits = 0)}] for \textsf{Large}, \Sexpr{formatC(ciNL[grepl("ce$", attributes), estimate], format = "f", digits = 0)} BDT [\Sexpr{formatC(as.numeric(ciNL[grepl("ce$", attributes), .(lb, ub)]), format = "f", digits = 0)}] for \textsf{Large grace}, \Sexpr{formatC(ciNL[grepl("le$", attributes), estimate], format = "f", digits = 0)} BDT [\Sexpr{formatC(as.numeric(ciNL[grepl("le$", attributes), .(lb, ub)]), format = "f", digits = 0)}] for \textsf{Cattle}) than the \textsf{Traditional} arm by period 4. 
...
In the \textsf{Traditional} arm, there are \Sexpr{sum(piga1[grepl("trad", Arm) & !grepl("D", membershipstatus) & !grepl("other", iga1), N])} borrowing members (\Sexpr{round((sum(piga1[grepl("trad", Arm) & !grepl("D", membershipstatus) & !grepl("other", iga1), N])/sum(piga1[grepl("trad", Arm) & !grepl("D", membershipstatus), N]))*100, 2)}\%) who report cattle as their first IGA, and \Sexpr{sum(piga1[grepl("trad", Arm) & !grepl("D", membershipstatus) & grepl("other", iga1), N])} borrowing members (\Sexpr{round((sum(piga1[grepl("trad", Arm) & !grepl("D", membershipstatus) & grepl("other", iga1), N])/sum(piga1[grepl("trad", Arm) & !grepl("D", membershipstatus), N]))*100, 2)}\%) who report other than cattle as their first IGA. 
This contrasts with the non-\textsf{Traditional} arms that \Sexpr{sum(piga1[!grepl("trad", Arm) & !grepl("D", membershipstatus) & !grepl("other", iga1), N])} borrowing members (\Sexpr{round((sum(piga1[!grepl("trad", Arm) & !grepl("D", membershipstatus) & !grepl("other", iga1), N])/sum(piga1[!grepl("trad", Arm) & !grepl("D", membershipstatus), N]))*100, 2)}\%)
who report cattle as their first IGA and \Sexpr{sum(piga1[!grepl("trad", Arm) & !grepl("D", membershipstatus) & grepl("other", iga1), N])} borrowing members (\Sexpr{round((sum(piga1[!grepl("trad", Arm) & !grepl("D", membershipstatus) & grepl("other", iga1), N])/sum(piga1[!grepl("trad", Arm) & !grepl("D", membershipstatus), N]))*100, 2)}\%) other than cattle as their first IGA. 
...
\vspace{2ex}
\hfil\includegraphics[width = 10cm]{\Sexpr{paste0(pathprogram, "figure/EstimationMemo/IncomeConsumptionEffects.pdf")}}\\
\renewcommand{\arraystretch}{1}
\setlength{\tabcolsep}{1pt}
...
@
	...
	When a member does not reach the due amount with installments, they had to repay from the (net) saving, an arrangement to which the lender and the borrowers made at the loan contract signment. Repayment rates at the end of period 4 after using net saving are \Sexpr{formatC(TabRepay[!grepl("over", Arm), FullRepayRate], digits = 2, format = "f")}\%, respectively, for \textsf{\Sexpr{TabRepay[!grepl("over", Arm), Arm]}} arms, \Sexpr{formatC(TabRepay[grepl("over", Arm), FullRepayRate], digits = 2, format = "f")}\% for overall, and \Sexpr{formatC((sum(TabRepay[!grepl("over|trad", Arm), yes])/sum(TabRepay[!grepl("over|trad", Arm), sum]))*100, digits = 2, format = "f")}\% for the average of non-\textsf{Traditional} lending arms. %(from \textsf{\footnotesize \Sexpr{paste0(DataFileNames[2], "InitialSample.rds")}}). 
...
Note:& Per period impacts ($\bfb_{3k}$ for period 3 and $\bfb_{4k}$ for period 4). \Sexpr{
#EstEqnText
...
<<echo = F>>=
SchImp <- 
  confis[grepl("Up", attributes) &grepl("change", ImpactType) & num >= 5 
    & grepl("Ta$", regtype) & grepl("^m", gender) & grepl("pr", school) & period > 2, ]
SchTrad <- 
  confis[(grepl("^Tra", attributes) & grepl("lev.* trad.*ea", ImpactType)) & num == 5
    & grepl("Ta$", regtype) & grepl("^m", gender) & grepl("pr", school) & period > 2, ]
@

	\SeiroAdded{yellow!50}{There is a possible negative impact for boys at the primary level in period 3 and 4 with the \textsf{Upfront} functional attribute. Point estimates on boys are mostly negative for other school levels with this functional atttribute, yet none of them are precisely estimated. Estimates on girls switch signs across specifications and arms. They have wider CIs than boys that include zero in their CIs, so they are incoherent, but a few estimates have reasonablly low $p$ values.} Negative impacts of boy's schooling may be due to a stronger household labor demand derived from increased cattle production in a household. This is in line with the finding in program rejection that the limited household size can be a constraint on participation. Cattle ownership naturally shifts the relative shadow prices in a household against child schooling, and the cattle rearing tasks are not so brawn intensive enough that even the primary school aged boys can handle. This may be a potential downside of having greater cattle production in a household. \SeiroAdded{yellow!50}{The estimated impacts on primary school aged boys range from \Sexpr{round(min(SchImp[period == 4, estimate]*100), 0)} to \Sexpr{round(max(SchImp[period == 4, estimate]*100), 0)} percentage points, which amounts to \Sexpr{round((min(SchImp[period == 4, estimate])/SchTrad[period == 4, estimate])*100, 1)} to \Sexpr{round((max(SchImp[period == 4, estimate])/SchTrad[period == 4, estimate])*100, 1)} percent of the period 4 mean enrollment rate of \textsf{Traditional} arm students at \Sexpr{round(SchTrad[period == 4, estimate]*100, 0)} percent.}
...
Note:&  Rows indicate regression specifications 1, 2, and 3. Columns indicate impacts of each arms relative to the \textsf{Traditional} arm. \textsf{Owner} is a group who holds cattle at the baseline, \textsf{Adi} is a group who has an experience of lease-in cattle contract at the baseline, and \textsf{None} are all other individuals. There are \Sexpr{OwnCattleAtBaseline} members who owned cattle at the baseline, \Sexpr{AdiCattleAtBaseline} members who ever practiced \textsf{Adi} at the baseline, and  \Sexpr{776-OwnCattleAtBaseline-AdiCattleAtBaseline} members who have no experience in cattle rearing. See also the footnotes of \textsc{Figure \ref{fig NetAssetEffects}}.\setlength{\baselineskip}{6pt}\\[2ex]
\end{tabular}
}
