...
@
There are \Sexpr{nrow(ar[grepl("tw|dou", TradGroup) & tee == 1, ])} members with TradGroup = twice, double. They were dropped from estimation sample. If \textsf{UseTrimmedSample==T}, attrition is based on all 800 members, if \textsf{F}, attrition is analysed using 786 members.
...
pne <- NeA1R[tee == 1 & if (UseTrimmedSample) !grepl("tw|dou", TradGroup), 
  .(hhid, tee, NarrowNetValue, NetValue)]
...
asv <- merge(as, pslv, by = c("hhid", "tee"), all.x = T)
...
<<permutation tests full sample, warning = F, eval = F>>=
# data names: ..af, ..rf 
...
<<permutation tests dropping group rejecters, warning = F, eval = F>>=
# data names: ..aj, ..rj
...
<<permutation tests dropping 24 twice double TradGroup members, warning = F, eval = F>>=
# data names: ..at, ..rt
