...
vr[, GroupStatus := factor(GroupStatus)]
vr <- vr[, .(groupid, VArm, GroupStatus, GroupComment)]
saveRDS(vr, paste0(pathsaveHere, "VillageAssignment.rds"))
@
...
<<use VArm to fill NAs in AssignOriginal in roster, echo = T>>=
...
<<baseline asset sample completeness, echo = T>>=
ass[, InBase := F]
...
ass <- ass[, .(gid, hhid, o800, Mstatus, 
  Arm, survey, DistDate1, IntDate, 
  ...
  AmountFilled)]
ros <- ros[, .(Arm, 
  gid, hhid, o800, Mstatus, Mgroup, ObPattern, AttritIn,
  IntDate, year, survey, 
...
<<define ad0>>=
ad0 <- adw3[, 
  .(groupid, hhid, TradGroup, TradGroup2,
...
adbasevars <- c("creditstatus", "Mem", "povertystatus", "DisDate1")
@

\subsection{Merge roster with admin files: \textsf{ar.1}}

Create \textsf{adbase} (baseline fixed characteristics \Sexpr{adbasevars} taken from \textsf{ad0} (={\footnotesize \Sexpr{gsub("\\_", "\\\\_", paste0(path1234, "admin_data_wide2.rds"))}}). Merge it with roster. This gives fixed characteristics of membership attached with roster. Name the resulting data as \textsf{ar.0}.
...
@
Tabulation of arms with \textsf{ar.0} for \textsf{o800}. There are \Sexpr{sum(is.na(ar.0[o800 == 1L, .(teenum =1:.N, Arm), by = .(hhid, survey)][survey == 1 & teenum == 1, Arm]))} NAs which will be filled in with \textsf{RCT\_village.dta} with \textsf{ar, arAll} in the next subsection.
<<By merging in this way I have>>=
addmargins(table0(ar.0[o800 == 1L, .(teenum =1:.N, Arm), by = .(hhid, survey)][
  teenum == 1, .(survey, Arm)]), 2)
@
...
@
In \textsf{roster + admin} (base: roster): Tabulate \textsf{hhid} observations by \textsf{survey} round and \textsf{Arm} before supplementing with \textsf{AssignOriginal} and \textsf{VArm}. Note: \Sexpr{nrow(ar.1[survey == 1 & MemNum == 1 & is.na(Arm), ])} observations with NA are also pointed in \textsf{read\_cleaned\_data.rnw} and are going to be dealt with in the next subsection.
...
for (s in 2:4)
  ar.00[
    ...
    , survey := as.integer(s)]
# dates after IntDate.4 is defined as rd 4
ar.00[Date > IntDate.4, survey := 4L]
ar.00[, grepout("IntD", colnames(ar.00)) := NULL]
ros.0[, tee := 1:.N, by = hhid]
ros.0 <- ros.0[tee == 1, .(groupid, hhid, Arm, o800,
...
table0(arAll[tee == 1 & grepl("old|iRej|^g", Mstatus) & 
  !grepl("tw|dou", TradGroup), .(survey, Arm)])
@
...
vr <- readRDS(paste0(pathsaveHere, "VillageAssignment.rds"))
...
ar[, tee := 1:.N, by = hhid]
addmargins(table0(ar[o800 == 1L & survey == 1 & MemNum == 1, .(Arm, VArm)]))
addmargins(table0(ar[survey == 1 & MemNum == 1, .(Arm, VArm)]))
@
...
<<tabulate RArm after supplementing with VArm, echo = T>>=
addmargins(table0(ar[o800 == 1L & MemNum == 1, .(survey, Arm)]))
addmargins(table0(ar[MemNum == 1, .(survey, Arm)]))
@
Below is what is supplemented from \textsf{VArm} of village level information to the \Sexpr{nrow(ar[o800 == 1L & MemNum == 1 & survey == 1 & is.na(Arm), ])} NAs.
...
arA <- vr[arAll]
arA[is.na(Arm) & !is.na(VArm), Arm := VArm]
...
@
If any: \Sexpr{nrow(ar[is.na(BStatus)&tee == 1&o800==1L, ])} NAs in \textsf{BStatus} are borrowers. Correct it.
<<>>=
if (any(ar[, is.na(BStatus)])) 
...
addmargins(table0(ar[tee == 1 & MemNum == 1 & o800 == 1L, .(Arm, AttritIn)]))
addmargins(table0(ar[o800 == 1L & MemNum == 1 & survey == 1, .(BStatus, AttritIn)]))
@
...
addmargins(table0(ar[o800 == 1L & MemNum == 1 & survey == 1 & 
  grepl("tra", Arm) & !grepl("tw|dou", TradGroup),
 .(BStatus, AttritIn)]))
@
...
In \textsf{ar}: Keep if \textsf{Mstatus} includes strings old, iRej, gEro, gRej, \& \textsf{TradGroup} does not include strings tw (relaxing  \textsf{DisDate1} is before 2015-01-01). There are \Sexpr{nrow(ar[o800 == 1L & grepl("old|iRej|^g", Mstatus) & !grepl("tw", TradGroup) & MemNum == 1 & survey == 1, ])} HHs at the baseline. \textcolor{blue}{This the admin data used in this note.} This also shows a lower attrition rate for \textsf{large} arm. \gobblepars
<<>>=
...
#    !grepl("tw", TradGroup) & MemNum == 1, .(survey, Arm)]), 1, sum))
<<tabulate Arm in roster plus admin 2, echo = T>>=
...
<<tabulate Mstatus against Arm at survey 1 and MemNum 1 >>=
ar[, MemNum := 1:.N, by = .(hhid, survey)]
addmargins( table0(ar[MemNum == 1 & survey == 1, .(Mstatus, Arm)]))
@
In \textsf{ar}, for \textsf{o800} we have:
<<tabulate Mstatus against Arm ratios o800>>=
addmargins(table0(ar[o800 == 1 & MemNum == 1 & survey == 1, .(Mstatus, Arm)]))
@
<<copy Arm across rounds>>=
ar[, Arm := Arm[!is.na(Arm)][1], by = groupid]
@
...
arA[, .Arm := paste0(toupper(substr(Arm, 1, 1)), substr(Arm, 2, 100))]
...
arA[, tee := 1:.N, by = hhid]
addmargins(table0(arA[o800 == 1L & tee == 1, .(Mstatus, Arm)]))
@
Create \textsf{LYear}.
<<create LYear>>=
...
vartoattach <- c("Arm", "TradGroup", 
  "Mem", "ObPattern", "AttritIn", "o1600", 
...
lvo[, .Arm := paste0(toupper(substr(Arm, 1, 1)), substr(Arm, 2, 100))]
...
cbind(table0(ar[tee == 1 & o800 == 1L, .(Mstatus, Arm)]), 
  table0(arA[tee == 1 & o800 == 1L, .(Mstatus, Arm)]))
@
...
addmargins(table0(arA[LoanYear == 1 & LoanMonth == 1 & !is.na(LoanYear) & o800 == 1L, 
  .(Mstatus, EverRepaid)]))
@
What is relevant in estimation is observations by \textsf{LoanYear}, total of \Sexpr{sum(table0(arA[LoanYear == 1 & LoanMonth == 1 & !is.na(LoanYear) & o800 == 1L,   .(LoanYear, Arm)]))}.
...
<<attrition in o800>>=
addmargins(table0(arA[LoanMonth == 1 & !is.na(LoanYear) & LoanYear == 1 & o800 == 1L, .(ObPattern, Arm)]))
@
\textsf{BorrowerStatus} pattern in original 800.
<<BorrowerStatus in o800>>=
addmargins(table0(arA[LoanMonth == 1 & !is.na(LoanYear) & LoanYear == 1 & o800 == 1L, .(BorrowerStatus, Arm)]))
@
\textsf{BStatus} pattern in original 800.
<<BStatus in o800>>=
addmargins(table0(arA[LoanMonth == 1 & !is.na(LoanYear) & LoanYear == 1 & o800 == 1L, .(BStatus, Arm)]))
@
...
for (s in samples){
  ...
  for (j in levels(ar[, Mstatus])) {
    for (m in levels(ar[, Arm])) {
      attseqN <- c(attseqN, 
        nrow(ar[
          MemNum == 1 & eval(parse(text=s)) == 1L & survey == 1 &
          grepl(j, Mstatus) & grepl(paste0(m, "$"), Arm) & AttritIn < 9L, ])
        )
      attseqT <- c(attseqT, 
        nrow(ar[
          MemNum == 1 & eval(parse(text=s)) == 1L & survey == 1 &
          grepl(j, Mstatus) & grepl(paste0(m, "$"), Arm), ])
        )
    }
  }
  ...
  AttSL <- reshape(AttSW, direction = "long", idvar = c("Arm", "Mstatus"),
...
g <- ggplot(subset(dt, !grepl("^at", variable)), 
  ...
  facet_grid(Sample ~ Arm, scales = "free_y")
ggsave(
  paste0(pathprogram, 
