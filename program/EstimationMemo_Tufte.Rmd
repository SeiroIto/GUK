...
link-citations: yes
#### c:\seiro\languages\R\R-4.2.1\bin\x64\Rscript.exe -e "path <- 'c:/data/GUK/analysis/program/'; rmarkdown::render('c:/data/GUK/analysis/program/EstimationMemo_Tufte.Rmd')"
#### path0 <- "c:/data/GUK/"; setwd(path <- paste0(path0, "analysis/")); pathprogram <- paste0(path, "program/"); rmarkdown::render(paste0(pathprogram, "EstimationMemo_Tufte.Rmd"))
#### rmarkdown::render(paste0(path, "program/EstimationMemo_Tufte.Rmd"), output_format = "pdf_document")
...
getwd()
here::i_am("GUK/analysis/program/EstimationMemo_Tufte.Rmd")
getwd()
...
In the estimation, there are `r length(FileNameHeader)` "reg types" in estimation, which are:  

* By arm (base)   
* By functional attribute   
* By arm and by poverty status   
* Time varying by arm   
* Time varying by functional attribute   
* Time varying by poverty class   
* Time varying by functional attribute and poverty class   

Within each type, there are `r length(additions)-1` regression specifications. In our analysis, we mainly rely on "Time varying by arm," and refer to "Time varying by functional attribute" whenever necessary. 
...
$$
\text{Time varying "impacts"} = \text{baseline "impacts" } b_{a_{0}} + \text{period }t\text{ changes }b_{ta_{0}} = 0.  
$$
...
ggsave(
  paste0(pathprogram, 
    "figure/EstimationMemo/MeanOutcomesByArmAndPeriod.png")
  , p,
