...
.TableOnLeft {
  #### CLAUDE tpo: "zero" is not valid CSS, changed to 0
...
library(knitr)
library(tufte)
#### AG: Add functions.R as requested by user
...
####     "individual rejection", "group rejection", "rejection by flood"))]
  ...
  dd2[, povertystatus := factor(povertystatus,
    levels = c("Ultra Poor", "Moderate Poor"),
...
for (dtyp in c("AdminData", "Trimmed")) 
{
  ...
  lab[, teenum := 1:.N, by = .(hhid, survey)]
  con[, tee := (1:.N)+1, by = hhid]
  #### CLAUDE dup: deleted a perfect duplicate of L685
  ####   deleted line was: lab[, teenum := 1:.N, by = .(hhid, survey)]
  ####   verified: no merge/join uses lab$teenum between L685 and here,
  ####   and nothing mutates lab in between (L686 touches con only).
  s1[, tee := survey]
  ar[, teenum := 1:.N, by = .(hhid, survey)]
  ar[, tee := survey]
  armtabs.o800 <- NULL
...
#### setnames(desMainN, "V1", "N") #### CLAUDE tpo: 2026-05-14
...
ktft <-   paste("Source: Estimated with GUK administrative", 
    ...
    "total - (rejected members + attrited members).",
    PrefTestsDefinitions1)
gttb <- gt::gt(as.data.frame(desM))
desM <- sapply(desM, function(x) gsub("\\(", "\\\\(", x))
kt <- kable(data.table(desM), format = "html", align = c("r", rep("c", 5)), escape = F)
kt <- column_spec(kt, 1, width = "6cm; min-width:6cm;")
kt <- column_spec(kt, 2:6, width = "3.5cm; min-width:3.5cm;")
