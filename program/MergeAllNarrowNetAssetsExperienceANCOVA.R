...
assstrings <- "^Arm$|^groupid$|hhid|tee|^.Asse|Narrow|^dummy.*[a-z]$|Floo|HHsize|Time\\.?.|Head|With|.Size|^Cum[ERN].*before$|^Debt.*before$|Bal|BSta|00|AttritIn|TradG|ObP"
...
NeA1R[, NarrowTotalValue := TotalImputedValue + NarrowHAssetAmount + PAssetAmount]
NeA1R[, Total2Value := TotalImputed2Value + HAssetAmount + PAssetAmount]
NeA1R[, NarrowTotal2Value := TotalImputed2Value + NarrowHAssetAmount + PAssetAmount]
...
NeA1R[, c("TotalImputedValue", "HAssetAmount", "NarrowHAssetAmount",
...
saveRDS(NeAE1, paste0(pathsaveHere, "NarrowNetAssetsExperienceRegData.rds"))
saveRDS(NeAE, paste0(pathsaveHere, "NarrowNetAssetsExperienceANCOVATrimmed.rds"))
saveRDS(NeAE2, paste0(pathsaveHere, "NarrowNetAssetsExperienceANCOVA.rds"))
saveRDS(netAD, paste0(pathsaveHere, "NarrowNetAssetsExperienceFigure.rds"))
write.tablev(NeAE2, paste0(pathsaveHere, "NarrowNetAssetsExperienceANCOVA.prn"), 
  colnamestrue = F)
...
NeAfig <- NeAFig[, .(Arm, groupid, hhid, UDdummyUltraPoor, tee, NarrowNetValue)]
