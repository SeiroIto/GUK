...
xid[, Fromxid := T]
datafiles <- c("s1", "ar", "ass", "lvo", "lvp", "lab", "far", "con")
Datafiles <- c("S1", "Ar", "Ass", "Lvo", "Lvp", "Lab", "Far", "Con")
DataFileNames <- c(
  "Schooling", "Repayment", "Asset", "Livestock", "LivestockProducts",
...
FileName <- "Asset"
FileNameHeader <- paste0(c("", "Grace", "PovertyStatus", "Size", "Attributes"), "OriginalHHs")
...
\renewcommand{\arraystretch}{.55}
\hfil\begin{tikzpicture}
\node (tbl) {\input{\Sexpr{paste0(pathsaveHere, FileName, FileNameHeader[1], "FDEstimationResults.tex")}}};
%\input{c:/dropbox/data/ramadan/save/tablecolortemplate.tex}
\end{tikzpicture}\\
\renewcommand{\arraystretch}{.8}
\setlength{\tabcolsep}{1pt}
...
library(ggplot2)
ggplot(data = subset(ALfig, !is.na(Arm)), aes(group = tee)) + 
...
g <- ggplot(data = subset(pc21, size_sequence == 1 & Seq > 1), 
...
ggsave(
  paste0(pathprogram, 
    "figure/ImpactEstimationOriginal1600Memo2/2ndOrLaterFixedInvestmentAmountByYear.png"),
  g,
...
\hfil\includegraphics{\Sexpr{  paste0(pathprogram, "figure/ImpactEstimationOriginal1600Memo2/2ndOrLaterFixedInvestmentAmountByYear.png")}}\\
\renewcommand{\arraystretch}{1}
\hfil\begin{tabular}{>{\hfill\scriptsize}p{1cm}<{}>{\scriptsize}p{12cm}<{\hfill}}
Source: & Survey data.\\
