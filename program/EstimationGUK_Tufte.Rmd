...
```{r TableFootnotes, file = "../program/TableFootnotesHTML.R", echo = F}
```
...
Covariates give up to 6 regression specifications (2--6 depending on the outcome).
<!-- CLAUDE com: 2026-07-16 was "5"; all saved ANCOVA_*.qs hold 2-6 specs, max 6 -->
...
[`ANCOVAEstimationFile3.R`](../program/ANCOVAEstimationFile3.R) does the following:  
...
```{r linhyp rexlog init, eval = T, echo = F}
#### Added by Claude
...
The table below shows one example slice of `confi` — the seven per-period
tests (net assets, `Ta` regression type, specification 1, `LargeSize` arm,
stored period 2 = survey period 3). `CombiOfCoeff` gives the coefficient
combination behind each linear-hypothesis vector (generic names: `Arm` =
`dummyLargeSize` here; `Time3` because stored period 2 + 1 = 3).

```{r confi example table, eval = T, warning = F, message = F}
library(tinytable)
...
* The `period` column of `confi` stores the loop index `i`; the reshape
  chunk later adds 1 (`period := period + 1`), so `period` 2 rows test
  survey period 3.
* Labels such as "level of trad in period X" keep `X` as a placeholder
  deliberately: the reshape chunk matches these exact strings in
  `factor(ImpactType, levels = )`, and the actual period is carried by the
  `period` column. Replacing `X` with a number in the construction code
  would send those rows to `NA`.
* Identities visible above: `NinT` $-$ `TinT` $=$ `dNinT`;
  `N2` $-$ `TL` $=$ `periNrelativeT`; `NinT2` $=$ `TinT` $+$ `periNrelativeT`.
* `periNrelativeT` $=$ `Arm + Arm*Time3` $=$ (nontrad $-$ trad gap in
  period 2) $+$ (change in the gap by period 3) $=$ the total
  nontrad $-$ trad gap in period 3. The "sum of" in its label refers to
  these two components, not to a sum over periods.
* `N2` $=$ `TL` $+$ `periNrelativeT` $=$ the level of nontrad in the
  period, per the intended combination (fixed 2026-07-16: the pre-fix
  code `hvT0 + hvNinT` omitted `Arm`, understating nontrad levels by the
  arm main effect).
* These seven tests repeat for every arm and every period in any
  regression type containing `T`; `a` types append the attribute (`j`)
  tests after them.

```{r linhyp map, echo = F, results = 'asis'}
...
<details><summary>Click here to see the code to reshape confi.</summary>
...
```{marginfigure}
Net asset impacts are estimated with ANCOVA. `Large/Upfront` , `Large grace`, and `Cattle` give impacts relative to `Traditional` arm. ``WithGrace` gives the grace period impacts relative to `Upfront`. `InKind` gives the impacts of managerial supports relative to the combination of `Upfront` and `WithGrace`.
```
...
```{marginfigure}
Schooling impacts by arm and by period. Impacts are relative to concurrent `Traditional` arm. `Traditional` shows raw enrollment rates of `Traditional` arm. `Large` , `Large grace`, and `Cattle` give impacts relative to concurrent `Traditional` arm.
```
...
```{marginfigure}
Consumption and labour income impacts. `Large/Upfront` , `Large grace`, and `Cattle` give impacts relative to `Traditional` arm. Consumption impacts are estimated with OLS, labour income impacts are estimated with ANCOVA. There are only two regression specifications for consumption, because we do not expect net asset at baseline or cattle holding at baseline to affect consumption changes. Results using these covariates give qualitatively the same results. 
```
