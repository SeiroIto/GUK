...
ar[, MonthGap := min(DisDate1, na.rm = T), by = groupid]
ar[MonthGap == Inf, MonthGap := NA]
ar[, MonthGap := round(
...
<<merge assets and livestock data, eval = F, cache = F, child='c:/data/GUK/analysis/program/MergeAssetsAndLivestockANCOVA.rnw'>>=
# source(paste0(pathprogram, "MergeAssetsAndLivestockANCOVA.R"))
# this gives NetAssetsANCOVATrimmed.rds
<<merge narrow assets livestock saving data for destat, cache = F, eval = F, child='c:/data/GUK/analysis/program/MergeAllNarrowNetAssetsANCOVA.rnw'>>=
...
iiM <- c("HeadLiteracy", "HeadAge", "HHsize", "FloodInRd1", 
  ...
  #"RNarrowNetValue", "RNetValue", "RBroadNetValue",
...
DestatMainByArm <- latextab(as.matrix(desM),
  ...
  hcenter = c(2.5, rep(1.05, ncol(desM)-1)), 
...
write.tablev(DestatMainByArm,  
  paste0(pathsaveHere, "DestatMainByArmNarrow.tex")
  , colnamestrue = F)
@
...
# Run MergeAllNarrowNetAssetsANCOVA.R first to produce various "Figure" data.
...
#bna <- nAD[, .(Arm, tee, BroadNetValue..mean, BroadNetValue..upper, BroadNetValue..lower, BroadNetValue..N)]
#rbna <- nAD[, .(Arm, tee, RBroadNetValue..mean, RBroadNetValue..upper, RBroadNetValue..lower, RBroadNetValue..N)]
...
# (cabinets, residential land, vcr/vcp, motorcycle, jewelry, are not recorded in baseline) 
...
library(ggplot2)
figD <- readRDS(paste0(pathsaveHere, "figD.rds"))
p <- ggplot(data = subset(figD, grepl("Con|^HHInc", file))
...
p <- p + 
  ...
  xlab("periods")
invisible(ggsave(
  paste0(pathprogram, "figure/EstimationMemo/MeanConsumptionIncomeByArmAndPeriod.png"),
  p,
...
library(ggplot2)
figD <- readRDS(paste0(pathsaveHere, "figD.rds"))
p <- ggplot(data = subset(figD, grepl("Con|Inc", file))
  , aes(x = factor(tee), y = mean, colour = Arm, shape = Arm, group = Arm)
  , size = 0.1, stroke = 0) + 
  geom_pointrange(aes(
    ymin = lower, ymax = upper), 
...
invisible(ggsave(
  paste0(pathprogram, "figure/EstimationMemo/MeanConsumptionIncomeDodge.png"),
  p,
...
\Sexpr{
knit_child(paste0(pathprogram, "subsubsection_BroadNetAssets.rnw"))
}
...
# have more than 20 members who are added after baseline)
...
\hfil\includegraphics[height = 5cm]{\Sexpr{  paste0(pathprogram, "figure/EstimationMemo/FirstIGAChoices.eps")}}\\
\renewcommand{\arraystretch}{1}
\hfil\begin{tabular}{>{\hfill\scriptsize}p{1cm}<{}>{\scriptsize}p{12cm}<{\hfill}}
Source: & Survey data.\\
...
\hfil\includegraphics[height = 5cm]{\Sexpr{    paste0(pathprogram, "figure/EstimationMemo/AllIGAChoices.eps")}}\\
\renewcommand{\arraystretch}{1}
\hfil\begin{tabular}{>{\hfill\scriptsize}p{1cm}<{}>{\scriptsize}p{12cm}<{\hfill}}
Source: & Survey data.\\
Note:&  \\[1ex]
...
\hfil\includegraphics[height = 5cm]{\Sexpr{    paste0(pathprogram, "figure/EstimationMemo/AllIGAChoicesCollapsed.eps")}}\\
\renewcommand{\arraystretch}{1}
\hfil\begin{tabular}{>{\hfill\scriptsize}p{1cm}<{}>{\scriptsize}p{12cm}<{\hfill}}
Source: & Survey data.\\
Note:&  \\[1ex]
...
FileNames <- c(#"Saving", 
  ...
  "AssetLivestock", "NetAssets", "BroadNetAssets",
...
library(multcomp)
# 1 is schooling
# 5 is number of cattle
# 8 is narrow net assets
# 11 is narrow net non livestock assets
for (r in 1:length(ListHeaderList)) {
  ...
  for (rr in 1:length(robj[[r]])) {
    ...
    regobj <- robj[[r]][[rr]]
    if (r == grep("cn", ListHeaderList)) regobj <- regobj[1:3] # consumption: 4 onwards is total consumption
    ...
    for (s in 2:length(regobj)) {
      ...
      # construct conf int for linear combination of coeffs
       ...
       ## baseline (=overall, when no Time is present) effects ##
      # trad
      # hypvecT0: picks covariates to test overall change
      #  [[1]]"\\(Intercept\\)"
      ...
      linhyp <- c(linhyp, list(hypvecT0, thisreg$coeff))
      names(linhyp)[length(linhyp)] <- 
        paste0(c(FileNames[r], regtype, s, "trad", "gross", startnum), collapse = "")
      # nontrad
      for (g in lattributes) {
        ...
        linhyp <- c(linhyp, list(hypvecN0, thisreg$coeff))
        names(linhyp)[length(linhyp)] <- 
          paste0(c(FileNames[r], regtype, s, g, "gross", startnum), collapse = "")
        # hypvecN1: difference of baseline Arm relative to baseline trad
        #  [[1]][2] "dummyInKind"
        cumstrings <- peristrings <- paste0("^", covadd.nontrad[[1]][2], "$")
        ...
        linhyp <- c(linhyp, list(hypvecN1, thisreg$coeff))
        names(linhyp)[length(linhyp)] <- 
          paste0(c(FileNames[r], regtype, s, g, "relative to concurrent trad", 
          startnum), collapse = "")
        # for cumulative, period 2 = relative to concurrent trad because it is the first period
        confi <- rbind(confi, 
          c(FileNames[r], regtype, s, g, "cumulative relative to concurrent trad", 
            startnum, confint(lhcow)$confint[1, ])
          )
        linhyp <- c(linhyp, list(hypvecN1, thisreg$coeff))
        names(linhyp)[length(linhyp)] <- 
          paste0(c(FileNames[r], regtype, s, g, "cumulative relative to concurrent trad", 
            startnum), collapse = "")
        # for period wise, period 2
        # for period wise, period 2 = relative to concurrent trad because it is the first period
        confi <- rbind(confi, 
          c(FileNames[r], regtype, s, g, "period wise relative to concurrent trad", 
            startnum, confint(lhcow)$confint[1, ])
          )
        linhyp <- c(linhyp, list(hypvecN1, thisreg$coeff))
        names(linhyp)[length(linhyp)] <- 
          paste0(c(FileNames[r], regtype, s, g, "period wise relative to concurrent trad", 
            startnum), collapse = "")
        ...
        if (grepl("T", regtype)) {
          ...
          for (i in (startnum+1):length(covadd.trad)) {
            ...
            linhyp <- c(linhyp, list(hypvecTinT, thisreg$coeff))
            names(linhyp)[length(linhyp)] <- 
              paste0(c(FileNames[r], regtype, s, "trad", "relative to baseline trad", i), collapse = "")
            ...
            linhyp <- c(linhyp, list(hypvec, thisreg$coeff))
            names(linhyp)[length(linhyp)] <- 
              paste0(c(FileNames[r], regtype, s, "trad", "gross", i), collapse = "")
            # nontrad
            ...
            linhyp <- c(linhyp, list(hypvecNinT, thisreg$coeff))
            names(linhyp)[length(linhyp)] <- 
              paste0(c(FileNames[r], regtype, s, g, "relative to baseline trad", i), collapse = "")
            # dhypvecNinT: Difference relative to concurrent trad
            # Marginal difference between g and trad in period T.
            ...
            linhyp <- c(linhyp, list(dhypvecNinT, thisreg$coeff))
            names(linhyp)[length(linhyp)] <- 
              paste0(c(FileNames[r], regtype, s, g, "relative to concurrent trad", i), collapse = "")
            # cumNrelativeT: Cumulative difference relative to concurrent trad
            # E.g., if X is dummyInKind, and time is 4
            #  "dummyInKind.Time2"+"dummyInKind.Time3"+"dummyInKind.Time4"
            # "dummyInKind" part is stored in cumstrings at hypvecN1
            cumNrelativeT <- rep(0, length(coeffvec))
            cumstrings <- paste(cumstrings, paste0("^", covadd.nontrad[[i]][2], "$"), sep = "|")
            cumNrelativeT[grepl(cumstrings, names(coeffvec))] <- 1*Mult
            lhcow <- glht(model=thisreg, linfct = matrix(cumNrelativeT, byrow = T, nrow=1), 
              alternative="two.sided", vcov.=thisV)
            confi <- rbind(confi, 
              c(FileNames[r], regtype, s, g, "cumulative relative to concurrent trad", 
                i, confint(lhcow)$confint[1, ])
              )
            linhyp <- c(linhyp, list(cumNrelativeT, thisreg$coeff))
            names(linhyp)[length(linhyp)] <- 
              paste0(c(FileNames[r], regtype, s, g, "cumulative relative to concurrent trad", i), collapse = "")
            # periNrelativeT: periodwise difference relative to concurrent trad
            # Total difference between g and trad in time X.
            #  "dummyInKind"+"dummyInKind.TimeX" for period X effects relative to trad in perid X
            # "dummyInKind" is stored in peristrings at hypvecN1
            periNrelativeT <- rep(0, length(coeffvec))
            peristrings2 <- paste(peristrings, paste0("^", covadd.nontrad[[i]][2], "$"), sep = "|")
            ...
            names(linhyp)[length(linhyp)] <- 
              paste0(c(FileNames[r], regtype, s, g, "period wise relative to concurrent trad", i), collapse = "")
            ...
            linhyp <- c(linhyp, list(hypvecN2, thisreg$coeff))
            names(linhyp)[length(linhyp)] <- 
              paste0(c(FileNames[r], regtype, s, g, "gross", i), collapse = "")
            ## 3 way (OwnCows0*Arm*Time) interactions ##
            # Loop over OwnCows0, AdiCows0
            if (grepl("sExpe", FileNames[r])) {
              for (j in paste0("dummy", c("Own", "Adi"), "Cattle0")) {
                # dhypvecJ0: Difference between j and trad at baseline
                #  dummyAdiCattle0
                # dhypvecJG0: Difference between j and g at baseline
                #  dummyAdiCattle0.Large
                # dhypvecJinT: Difference between j and concurrent trad in period T.
                #  dummyAdiCattle0.TimeX
                # dhypvecJGinT: Difference between j*g and g in T
                #  dummyAdiCattle0.Large.TimeX
                # hypvecJinT: Total difference between j and concurrent trad in period T.
                #  dummyAdiCattle0 + dummyAdiCattle0.TimeX
                # hypvecJGinT: Total difference between j*g and g in T
                #  dummyAdiCattle0.Large + dummyAdiCattle0.Large.TimeX
                # periJGinT: Total difference between j*g and concurrent trad in time X.
                #  dummyLarge + dummyLarge.TimeX : hypvecNinT
                # +dummyAdiCattle0 + dummyAdiCattle0.TimeX : hypvecJinT
                # +dummyAdiCows0.Large+dummyAdiCows0.Large.TimeX : hypvecJGinT
                # dhypvecJ0: dummyAdiCattle0
                 dhypvecJ0 <- rep(0, length(coeffvec))
                 dhypvecJ0[grepl(
                     paste0("^", j, "$")
                   , names(coeffvec))] <- 1*Mult
                 lhcow <- glht(model=thisreg, linfct = matrix(dhypvecJ0, byrow = T, nrow=1), 
                   alternative="two.sided", vcov.=thisV)
                 confi <- rbind(confi, 
                   c(FileNames[r], regtype, s, j, "relative to baseline trad", 
                     i, confint(lhcow)$confint[1, ])
                   )
                 linhyp <- c(linhyp, list(dhypvecJ0, thisreg$coeff))
                 names(linhyp)[length(linhyp)] <- 
                   paste0(c(FileNames[r], regtype, s, j, "relative to baseline trad", i), collapse = "")
                # dhypvecJinT: dummyAdiCattle0.TimeX
                 dhypvecJinT <- rep(0, length(coeffvec))
                 dhypvecJinT[grepl(
                     paste0("^", j, ".Time.$")
                   , names(coeffvec))] <- 1*Mult
                 lhcow <- glht(model=thisreg, linfct = matrix(dhypvecJinT, byrow = T, nrow=1), 
                   alternative="two.sided", vcov.=thisV)
                 confi <- rbind(confi, 
                   c(FileNames[r], regtype, s, j, "relative to concurrent trad", 
                     i, confint(lhcow)$confint[1, ])
                   )
                 linhyp <- c(linhyp, list(dhypvecJinT, thisreg$coeff))
                 names(linhyp)[length(linhyp)] <- 
                   paste0(c(FileNames[r], regtype, s, j, "relative to concurrent trad", i), collapse = "")
                # hypvecJinT: Total difference between j and concurrent trad in period T.
                #  dummyAdiCattle0 + dummyAdiCattle0.TimeX
                 hypvecJinT <- dhypvecJ0+dhypvecJinT
                # hypvecJG0: dummyAdiCattle0.Large
                 hypvecJG0 <- rep(0, length(coeffvec))
                 hypvecJG0[grepl(
                     paste0("^", j, "$")
                   , names(coeffvec))] <- 1*Mult
                # dhypvecJGinT: dummyAdiCattle0.Large.TimeX
                 dhypvecJGinT <- rep(0, length(coeffvec))
                 dhypvecJGinT[grepl(
                     paste0("^", j, ".", covadd.nontrad[[i]][2])
                   , names(coeffvec))] <- 1*Mult
                # hypvecJGinT: dummyAdiCattle0.Large + dummyAdiCattle0.Large.TimeX
                 hypvecJGinT <- hypvecJG0 + dhypvecJGinT
                 lhcow <- glht(model=thisreg, linfct = matrix(hypvecJGinT, byrow = T, nrow=1), 
                   alternative="two.sided", vcov.=thisV)
                 confi <- rbind(confi, 
                   c(FileNames[r], regtype, s, j, "relative to concurrent arm/attribute", 
                     i, confint(lhcow)$confint[1, ])
                   )
                 linhyp <- c(linhyp, list(hypvecJGinT, thisreg$coeff))
                 names(linhyp)[length(linhyp)] <- 
                   paste0(c(FileNames[r], regtype, s, j,
                     "RelativeToConcurrentArmAttribute", i), collapse = "")
                # periJGinT: Total difference between j*g and concurrent trad in time X.
                #  dummyLarge + dummyLarge.TimeX : hypvecNinT
                # +dummyAdiCattle0 + dummyAdiCattle0.TimeX : hypvecJinT
                # +dummyAdiCows0.Large+dummyAdiCows0.Large.TimeX : hypvecJGinT
                periJGinT <- hypvecNinT+hypvecJinT+hypvecJGinT
                lhcow <- glht(model=thisreg, linfct = matrix(periJGinT, byrow = T, nrow=1), 
                  alternative="two.sided", vcov.=thisV)
                confi <- rbind(confi, 
                  c(FileNames[r], regtype, s, j, "period wise relative to concurrent trad", 
                    i, confint(lhcow)$confint[1, ])
                  )
                linhyp <- c(linhyp, list(periNrelativeT, thisreg$coeff))
                names(linhyp)[length(linhyp)] <- 
...
confi[, ImpactType := factor(ImpactType, 
  levels = c("relative to baseline trad", "relative to concurrent trad", 
    "relative to concurrent arm/attribute",
    "cumulative relative to concurrent trad", "period wise relative to concurrent trad", 
    "gross"))]
...
confi[, attributes := factor(attributes, 
  ...
  "LargeSize", "WithGrace", "InKind", "dummyAdiCattle0", "dummyOwnCattle0"))]
confi[, attributes := factor(attributes, 
  labels = c("traditional", "Large", "LargeGrace", "Cattle",
  "Upfront", "WithGrace", "InKind", "AdiCattle", "OwnCattle"))]
...
confi[grepl("^Broad.*ts$", FileName), regressand := "broad net assets"]
...
confi[, regressand := factor(regressand, levels = 
  c("land", "livestock", "broad total assets", "net non livestock assets", 
    paste0(rep(c("net assets", "cattle"), each = 5), 
      rep(c("", ", Adi", ", None", ", Own", ", Experience"), 2))
    , "broad net assets", "net assets, AP", 
    "enrollment", "consumption", "labour incomes")
      )]
...
<<confi2 only cumulative impacts>>=
confi <- readRDS(paste0(pathsaveHere, "EstimatesCI.rds"))
confi1 <- confi[grepl("Lan|Num|^Net|NL|^BroadNetAssets$|Price", FileName) & 
  !grepl("Upfront", attributes), ]
confi1[grepl("^Large$", attributes), attributes := "Large/Upfront"]
confi1[, attributes := factor(attributes, levels = 
  c("traditional", "Large/Upfront", "LargeGrace", "Cattle", "WithGrace", "InKind",
    "AdiCattle", "OwnCattle"))]
confi1[, attributes := factor(attributes, labels = 
  c("Traditional", "Large/Upfront", "LargeGrace", "Cattle", "WithGrace", "InKind",
    "AdiCattle", "OwnCattle"))]
...
confi12 <- confi1[!grepl("^Tra", attributes) & grepl("cum", ImpactType), ]
confi2 <- rbind(confi11, confi12)
cols <- c("regtype", "attributes", "ImpactType")
confi2[, (cols) := droplevels(.SD), .SDcols = cols]
saveRDS(confi2, paste0(pathsaveHere, "confi2.rds"))
...
confi1 <- confi1[grepl("^peri", ImpactType), ]
confi1 <- confi1[grepl("Ta?$", regtype), ]
confi1[grepl("^Large$", attributes), attributes := "Large/Upfront"]
...
ggsave(
  paste0(pathprogram, 
  "figure/EstimationMemo/NetAssetLandCattleRelativeToConcurrentTradEffects.png"),
  p,
  width = 13, height = 12, units = "cm",
  dpi = 300
 )
pdf(
  paste0(pathprogram, 
  "figure/EstimationMemo/NetAssetLandCattleRelativeToConcurrentTradEffects.pdf")
  , width = 2*12/2.54, height = 2*8/2.54)
print(p)
whatever <- dev.off()
...
library(ggplot2)
confi <- readRDS(paste0(pathsaveHere, "EstimatesCI.rds"))
confie1 <- confi[grepl("^NetA.*ts", FileName) & !grepl("Exp|Pr", FileName) & 
  grepl("^peri", ImpactType) & grepl("^Ta?$", regtype) & !grepl("Trad", attributes), ]
confie1[, attributes := factor(attributes, labels = 
  c("Large/Upfront", "LargeGrace", "Cattle", "Upfront",
    "WithGrace", "InKind"))]
confie2 <- confie1[!grepl("^Up", attributes), ]
cols <- c("FileName", "regressand", "attributes")
confie2[, (cols) := droplevels(.SD), .SDcols = cols]
confie2[, SubGroup := factor(gsub(".*ts\\, ", "", regressand))]
confie2[, SubGroup := factor(SubGroup, levels = c("net assets", "Own", "Adi", "None"))]
confie2[, SubGroup := factor(SubGroup, labels = c("All", "Own", "Adi", "None"))]
confie2[, num := factor(num)]
# keep only 
cols <- c("FileName", "regressand", "attributes")
confie2[, (cols) := droplevels(.SD), .SDcols = cols]
...
ggsave(
  paste0(pathprogram, 
  "figure/EstimationMemo/NetAssetsByExperienceRelativeToConcurrentTradEffects.png")
  , p,
...
pdf(
  paste0(pathprogram, 
  "figure/EstimationMemo/NetAssetsByExperienceRelativeToConcurrentTradEffects.pdf")
  , width = 2*12/2.54, height = 2*8/2.54)
print(p)
whatever <- dev.off()
...
library(ggplot2)
confi <- readRDS(paste0(pathsaveHere, "EstimatesCI.rds"))
confie1 <- confi[grepl("^Num", FileName) & !grepl("Exp", FileName) & 
  grepl("^peri", ImpactType) & grepl("^Ta?$", regtype) & !grepl("Trad", attributes), ]
confie1[, attributes := factor(attributes, labels = 
  c("Large/Upfront", "LargeGrace", "Cattle", "Upfront",
    "WithGrace", "InKind"))]
...
cols <- c("FileName", "regressand", "attributes")
confie2[, (cols) := droplevels(.SD), .SDcols = cols]
confie2[, SubGroup := factor(gsub(".*tle\\, ", "", regressand))]
confie2[, SubGroup := factor(SubGroup, levels = c("cattle", "Own", "Adi", "None"))]
confie2[, SubGroup := factor(SubGroup, labels = c("All", "Own", "Adi", "None"))]
confie2[, num := factor(num)]
# keep only 
cols <- c("FileName", "regressand", "attributes")
...
print(p)
whatever <- dev.off()
<<figure NumCows experience relative to concurrent trad, warning = F, message = F, fig.align='center', fig.height = 12, fig.width = 8, fig.lp = 'Figure '>>=
library(ggplot2)
confi <- readRDS(paste0(pathsaveHere, "EstimatesCI.rds"))
confie1 <- confi[grepl("^Num.*Ex", FileName) &
  !grepl("Adi|Own", attributes) & grepl("^peri", ImpactType) & grepl("^Ta?$", regtype) & 
  !grepl("Trad", attributes), ]
confie1[, attributes := factor(attributes, labels = 
  c("Large/Upfront", "LargeGrace", "Cattle", "Upfront",
    "WithGrace", "InKind"))]
confie2 <- confie1[!grepl("^Up", attributes), ]
confie2[, ConditionOnAdi0 := "Not conditioned on Adi"]
confie2[num >= 3 & grepl("Expe", FileName), ConditionOnAdi0 := "Conditioned on Adi"]
cols <- c("FileName", "regressand", "attributes")
confie2[, (cols) := droplevels(.SD), .SDcols = cols]
cols <- c("num", "ConditionOnAdi0")
confie2[, (cols) := lapply(.SD, factor), .SDcols = cols]
p <- ggplot(data = confie2
  , aes(x = factor(period), y = estimate, 
      colour = num, shape = num, group = num), size = .1) + 
  geom_pointrange(aes(
...
ggsave(
  paste0(pathprogram, 
  "figure/EstimationMemo/NumCowsExperienceRelativeToConcurrentTradEffects.png")
  , p,
  width = 12, height = 8, units = "cm",
  dpi = 300
 )
pdf(
  paste0(pathprogram, 
  "figure/EstimationMemo/NumCowsExperienceRelativeToConcurrentTradEffects.pdf")
  , width = 2*12/2.54, height = 2*8/2.54)
print(p)
whatever <- dev.off()
...
\hfil\includegraphics[width = 14cm]{\Sexpr{    paste0(pathprogram, "figure/EstimationMemo/AssetsByPeriod.png")}}\\
\renewcommand{\arraystretch}{1}
\hfil\begin{tabular}{>{\hfill\scriptsize}p{1cm}<{}>{\scriptsize}p{12cm}<{\hfill}}
...
\hfil\includegraphics[width = 14cm]{\Sexpr{    paste0(pathprogram, "figure/EstimationMemo/AssetsByPeriodNon800.png")}}\\
\renewcommand{\arraystretch}{1}
\hfil\begin{tabular}{>{\hfill\scriptsize}p{1cm}<{}>{\scriptsize}p{12cm}<{\hfill}}
...
\hfil\begin{tabular}{>{\hfill\scriptsize}p{1cm}<{}>{\scriptsize}p{12cm}<{\hfill}}
Source: & Estimated with survey data.\\
Note:&  Cumulative impacts on various net asset measures. \Sexpr{c(CumImpactText3, FNAssetDefinitions)}\\[1ex]
\end{tabular}
}
...
ggsave(
  paste0(pathprogram, 
    "figure/EstimationMemo/CattleHoldingByArm.png")
  , g,
  width = 14, height = 10, units = "cm",
  dpi = 300
 )
...
\hfil\includegraphics{\Sexpr{  paste0(pathprogram, "figure/EstimationMemo/CattleHoldingByArm.png")}}\\
\renewcommand{\arraystretch}{1}
\hfil\begin{tabular}{>{\hfill\scriptsize}p{1cm}<{}>{\scriptsize}p{12cm}<{\hfill}}
Source: & Survey data.\\
Note:& Numbers of survey participants are \Sexpr{lvo[o800==1 & survey == 1, .(N = .N), by = .(Arm)][, N]} for \textsf{\Sexpr{unique(as.character(arid[, Arm]))}} arms in round 1, respectively. Holders rates are the number of cattle owners per arm size, holding size is average holding per owner, initial owner holding are average holding per owner who held cattle at baseline, and per capita holding is cattle owned per arm member. Initial owner holding and holder rates show impacts on the intensive and extensive margins, respectively. Per capita holding shows the total impacts on cattle holding.\\[1ex]
\end{tabular}
}
