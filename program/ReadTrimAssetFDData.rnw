...
CovStrings <- "^groupid$|hhid|tee|^dummy.*[a-z]$|Floo|Time\\.?.|With|.Size|Head|^creditstatus$|^Arm$|BSta|"
...
#     use.var.name.for.dummy.prefix = F, print.messages = F)
   dl <- FirstDiffPanelData(X = get(datas0[i]), 
     ...
     LevelCovariates = "^dummy|Head|^Time\\..$|Female$|Floo|Eldest|^cred.*s$|xid$|SchPa|^Arm$|BSta")
  dat <- dl$diff
