...
ar1 <- ar[, 
  ...
  colnames(ar)), with = F]
ar1[, grepout("UD|[mM]issw|^Time$", colnames(ar1)) := NULL]
...
dl <- FirstDiffPanelData(X = ar1[!((hhid == 7096302 & tee == 1) | (hhid == 7096303 & tee == 1)), ], 
  Group = "^hhid$", TimeVar = "^tee$", Cluster = "groupid",
  LevelCovariates = "^dummy|Head|^Time\\..$|Female$|Floo|Eldest|^Size$|RAr|^cred.*s$|xid$|Sch.*Pa")
dar1 <- dl$diff
dar1[, grepout("^en$", colnames(dar1)) := NULL]
dl <- FirstDiffPanelData(X = ar1, 
  Group = "^hhid$", TimeVar = "^tee$", Cluster = "groupid",
  LevelCovariates = "^dummy|Head|^Time\\..$|Female$|Floo|Eldest|^Size$|RAr|^cred.*s$|xid$|Sch.*Pa")
dar2 <- dl$diff
dar2[, grepout("^en$", colnames(dar2)) := NULL]
datas <- c("dar1", "dar2")
