We use the `initial' sample (has only 776 members after dropping members who received loans only twice), not the `trimmed' sample (has all 800 members). To set to the trimmed sample, set the parameter \textsf{UseTrimmedSample} to T. Here, we set to F.
<<set UseTrimmedSample TestMedian in FormSample.R, echo = T>>=
UseTrimmedSample <- F
TestMedian <- F
<<merge narrow assets livestock saving data forming sample, cache = F, child='c:/data/GUK/analysis/program/MergeAllNarrowNetAssetsANCOVA.rnw'>>=
#  source(paste0(pathprogram, "MergeAllNarrowNetAssetsANCOVA.R"))
# this gives NarrowNetAssetsANCOVATrimmed.rds
<<read trimmed sample files>>=
...
<<Tee is last observed round, eval = F>>=
...
<<correct AttritIn for 24 members>>=
addmargins(table(ar[tee == 1 & grepl("tw|dou", TradGroup), AttritIn]))
#ar[Tee == 1 & AttritIn == 9 & grepl("tw|dou", TradGroup), AttritIn := 2L]
@
There are \Sexpr{nrow(ar[grepl("tw|dou", TradGroup) & tee == 1, ])} members with TradGroup = twice, double. They were dropped from estimation sample. If \textsf{UseTrimmedSample==T}, attrition is based on all 800 members, if \textsf{F}, attrition is analysed using 776 members.
<<drop 24 members if UseTrimmedSample is F, eval = T, echo = T>>=
if (!UseTrimmedSample) ar <- ar[!grepl("tw|dou", TradGroup), ]
addmargins(table0(ar[o800 == 1L & tee == 1, .(Tee, AttritIn)]))
@
<<define psas and pslv>>=
psas <- ass[o800 == 1 & tee == 1, 
  .(hhid, tee, HAssetAmount, RHAssetAmount, PAssetAmount)]
pslv <- lvo[o800 == 1 & tee == 1, 
  .(hhid, tee, TotalImputedValue, NumCows)]
nne <- NeA1R[o800 == 1 & tee == 1, 
  .(hhid, tee, 
  RNetValue, RNarrowNetValue, RBroadNetValue,
  NetValue, NarrowNetValue, BroadNetValue)]
<<merge ar psas pslv>>=
...
<<Attrited Rejected GRejected IRejected keep 1st obs>>=
