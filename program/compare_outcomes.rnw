...
opts_chunk$set(fig.path='figure/compare_outcomes', cache.path='cache/compare_outcomes', cache = F, echo = T, results = 'markup', tidy.opts=list(blank=FALSE, width.cutoff=40))
...
tr[assignment == "", assignment := "quit after randomise"]
tr0 <- tr[, .(gid, hhid, memstatus, assignment, arm, accept, receivedCredit, 
	elapsed, daysFromStart)]
tb <-  table(tr0[, .(accept, assignment)])
tb2 <-  table(tr0[, .(memstatus, assignment)])
@
Treatment assignment file. There are \Sexpr{sum(tb2[, grepl("qu", colnames(tb2))])} cases of rejections who are group rejections (\Sexpr{tb2[grepl("gr.*rej", rownames(tb2)), grepl("qu", colnames(tb2))]}), lost to flood (\Sexpr{tb2[grepl("lost", rownames(tb2)), grepl("qu", colnames(tb2))]}), and old members (\Sexpr{tb2[grepl("old", rownames(tb2)), grepl("qu", colnames(tb2))]}). 
<<read treatment assignment prn>>=
setwd(pathsave)
tr <- fread("treatment_assignment.prn")
...
@
These 2 subjects who \textsf{accept}ed but NA in \textsf{receivedCredit}, \Sexpr{asc(tr0[grepl("old", memstatus) & is.na(assignment), hhid])} are under cow arm. 
<<>>=
...
<<echo = F, fig.align='center', fig.height = 2, fig.width = 4, fig.cap = "3 meals per day", fig.lp = 'Figure'>>=
...
<<echo = F, fig.align='center', fig.height = 2, fig.width = 3, fig.cap = "Stated use of credit in rd 2", fig.lp = 'Figure'>>=
...
Work hours.
<<echo = F, fig.align='center', fig.height = 2, fig.width = 3, fig.cap = "Work hours", fig.lp = 'Figure'>>=
...
<<echo = T, warning = F, fig.align='center', fig.height = 2, fig.width = 3, fig.cap = "New loans", fig.lp = 'Figure'>>=
...
<<echo = T, warning = F, fig.align='center', fig.height = 2, fig.width = 3, fig.cap = "Assets by eventual treatmemt status", fig.lp = 'Figure'>>=
...
@
<<echo = T, warning = F, fig.align='center', fig.height = 2, fig.width = 3, fig.cap = "Assets by disbursement status", fig.lp = 'Figure'>>=
asset <- Asset01t[assetNumber == 1, ]
...
<<echo = T, warning = F, fig.align='center', fig.height = 2, fig.width = 3, fig.cap = "Assets by elapsed days from disbursement, ATT", fig.lp = 'Figure'>>=
asset <- Asset01t[assetNumber == 1, ]
...
<<echo = T, warning = F, fig.align='center', fig.height = 3, fig.width = 3, fig.cap = "Assets by elapsed days from disbursement, ATT scatter plot", fig.lp = 'Figure'>>=
asset <- Asset01t[assetNumber == 1, ]
asset.ss <- subset(asset, assetNPV > 0 & !is.na(gid) & !is.na(elapsed) & receivedCredit)
library(ggplot2)
