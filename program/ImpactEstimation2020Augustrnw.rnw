...
% \end{mdframed}
	...
	By complementing this result with the contents of borrower IGAs that upfront liquidity is associated with cattle purchases, we conclude there is a poverty trap. In our study, the source of nonconvexity is the heifer price that is about three times more than the goat price and is not readily affordable for many residents. We also find that one can escape the poverty trap by frontloading the liquidity in lending.
	
	\textbf{Commercial viability of ultra poor programs} Lastly, selecting the ultra poor as the population to provide supports have often involved free consultation/training and transfers in the past. A few studies of ultra poor transfer programs report sustained increase in assets and incomes \citep{Blattman2014, BanerjeeetalScience2015, Blattmanetal2016, HaushoferShapiro2016}. A transfer program in the northern Bangladesh shows an occupational change and an income increase \citep{BandieraBRAC2017} and long-run asset accumulation \citep{Balboni2020}. %\citet{Kaboski2018Indivisibility} uses a lab-in-the-field experiment to show the link between investment indivisibility, saving, and patience. 
