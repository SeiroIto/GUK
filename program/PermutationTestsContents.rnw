...
# which is in PermutationTestsContents5.rnw
...
# Tests, for each variable, Arm i mean to be different from rest of other Arm means.
# Permute group labels to get permuted distribution of Arm i means. If Arm i = Arm non i, 
# then p values are around 50%.
set.seed(20180802)
...
for (i in 1:length(vartobetestedM)) 
  Pmtresults <- rbind(Pmtresults, 
    ...
    #above is min(pvalue(pmresults[[i]], method = "step-down"))
      "mean", tabres[i, -1]),
    c("", "$p$ values",  
    # below gives p values of null of "Arm a == non-a" for each "a", 
    # returns length(Arms)-1 vector of p values
...
Pmtresults[grepl("Ass|Liv|Net|Val", variables),  
  (cols) := lapply(.SD, 
...
Pmtresults <- rbind(Pmtresults, 
  t(c("N", "", as.data.frame.table(table(asv[, Arm]))[, 2]))
  , use.names = F)
...
asv[, TradArmRatio := as.numeric(grepl("trad", Arm))]
vartobetestedH <- vartobetested[!grepl("Arm|Impu|Index", vartobetested)]
#vartobetestedH[grep("Arm", vartobetestedH)] <- "TradArmRatio"
library(coin)
pm0results <- tab0results <- vector(mode = "list", length(vartobetestedH))
set.seed(20180802)
for (i in 1:length(vartobetestedH)) {
  pm0results[[i]] <- independence_test(eval(parse(text=
    paste(vartobetestedH[i], "~ Arm"))), data = asv,
    distribution = approximate(B=PermRepTimes))
  tab0results[[i]] <- 
  asv[, 
    .(mean(eval(parse(text=vartobetestedH[i])), na.rm = T)), by = Arm]
}
names(pm0results) <- names(tab0results) <- vartobetestedH
tab0res <- rbindlist(lapply(tab0results, 
  function(x) {tx <- data.table(t(x[, 2])); setnames(tx, as.character(unlist(x[,1]))); tx}))
tab0res <- formatC(as.matrix(tab0res), digits = 4, format = "f")
tab0res <- cbind(variable = vartobetestedH, tab0res)
#pmresults[[1]]@statistic@teststatistic
Pmt0results <- NULL
for (i in 1:length(vartobetestedH)) 
  Pmt0results <- rbind(Pmt0results, 
    c(vartobetestedH[i], "mean", tab0res[i, -1]),
    c("", "$p$ values",  
