...
<<merge assets livestock saving data forming sample, cache = F, child='c:/data/GUK/analysis/program/ComputeNetAssetsANCOVA.rnw'>>=
...
@
There are \Sexpr{nrow(ar[grepl("tw|dou", TradGroup) & tee == 1, ])} members with TradGroup = twice, double. They were dropped from estimation sample. If \textsf{UseTrimmedSample==T}, attrition is based on all 800 members, if \textsf{F}, attrition is analysed using \Sexpr{nrow(ar[o800 == 1L & tee == 1, ])-nrow(ar[o800 == 1L & grepl("tw|dou", TradGroup) & tee == 1, ])} members. We use the `initial' sample (has only \Sexpr{nrow(ar[o800 == 1L & tee == 1, ])-nrow(ar[o800 == 1L & grepl("tw|dou", TradGroup) & tee == 1, ])} members after dropping members who received loans only twice), not the `trimmed' sample (has all \Sexpr{nrow(ar[o800 == 1L & tee == 1, ])} members). \gobblepars
<<set UseTrimmedSample TestMedian in FormSample.rnw>>=
...
@
Out of \Sexpr{nrow(ar[tee==1 & o800==1, ])} members, there are \Sexpr{nrow(ar[tee==1 & o800==1 & AttritIn<9, ])} members who attrited.
